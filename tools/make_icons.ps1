...
function Draw($size, $path, $maskable, $opaque) {
  ...
  if ($opaque) { $g.Clear($green) } else { $g.Clear([System.Drawing.Color]::Transparent) }
  $s = $size / 512.0
...
Draw 192 (Join-Path $root 'icon-192.png') $false $false
Draw 512 (Join-Path $root 'icon-512.png') $false $false
Draw 512 (Join-Path $root 'icon-maskable-512.png') $true $false
Draw 180 (Join-Path $root 'apple-touch-icon.png') $false $true
